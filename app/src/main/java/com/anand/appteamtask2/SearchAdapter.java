...
public class SearchAdapter extends RecyclerView.Adapter<SearchAdapter.myViewHolder>{
    ...
    public SearchAdapter(ComplexSearchModel res) {
        this.res = res;
    }
    ...
    public void onBindViewHolder(@NonNull myViewHolder holder, int position) {
        holder.foodName.setText(res.getResults().get(position).getTitle());
        Glide.with(holder.itemView.getContext()).load(res.getResults().get(position)
                .getImage()).into(holder.foodImage);
        holder.id = res.getResults().get(position).getId();
        holder.prepTime.setText(String.format("Prep Time: %dmin", res.getResults().get(position).getReadyInMinutes()));
        holder.servings.setText(String.format("Servings: %d", res.getResults().get(position).getServings()));
        holder.estCost.setText(String.format("Est. Cost: %.2f", res.getResults().get(position).getPricePerServing()));
        holder.healthScore.setText(String.format("Health Score: %.1f", res.getResults().get(position).getHealthScore()));
        if (res.getResults().get(position).isVegetarian()){
